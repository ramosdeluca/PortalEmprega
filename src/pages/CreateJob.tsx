...
export default function CreateJob() {
  ...
  const fetchJobDetails = async () => {
    try {
      const data = await api.jobs.get(id as string);
      setFormData({
        title: data.title || '',
        cbo_code: data.cbo_code || '',
        cbo_name: data.cbo_name || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        await api.company.updateJob(id as string, formData);
        navigate('/company/jobs');
      } else {
      ...
      <div className="bg-white p-8 md:p-12 rounded-[2.5rem] border border-zinc-100 shadow-sm space-y-10">
        <div className="space-y-2">
          ...
          <p className="text-zinc-500">
            {isEditing ? 'Atualize os detalhes da vaga abaixo.' : 'Preencha os detalhes abaixo para atrair os melhores candidatos.'}
          </p>
        </div>
        ...
        {isFetching ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            >
              {loading ? 'Salvando...' : <><CheckCircle className="w-5 h-5" /> {isEditing ? 'Salvar Vaga' : 'Publicar Vaga'}</>}
            </button>
          </form>
        )}
      </div>
